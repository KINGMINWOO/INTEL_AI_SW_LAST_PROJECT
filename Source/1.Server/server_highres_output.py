    ...
    while True:
        try:
            ...
            if msg_size == 0:
                ...
                # # ✅ 라인 미감지 시 dump@done 재시도
                # if lowered == "line_not_found" and is_robot:
                #     print(f"{client_id} 라인을 찾지 못함 → dump@done 재시도 수행.")
                #     enqueue_nav2_sequence(
                #         client_id.upper(),
                #         [(
                #             "dump@done",
                #             COMMAND_SPECS["dump@done"]["target"],
                #             COMMAND_SPECS["dump@done"]["yaw"],
                #             "nav2",
                #         )],
                #     )
                #     continue
