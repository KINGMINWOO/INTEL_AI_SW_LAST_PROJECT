...
def register_text_client(client_id: str, conn: socket.socket) -> None:
    with text_lock:
        text_clients[client_id.upper()] = conn

def unregister_text_client(client_id: str) -> None:
    with text_lock:
...
def try_route_bracket_message(sender_id: str, line: str) -> bool:
    ...
    if end <= 1:
        return False
    target = line[1:end].strip()
    payload = line[end + 1 :].lstrip()
    ok, reason = _send_text_to(target, sender_id, payload)
    if ok:
        print(f"[ROUTE] {sender_id} -> {target}: {payload}")
    else:
        print(f"[ROUTE-ERR] {sender_id} -> {target}: {reason}")
    return True
    ...
    while True:
        try:
            ...
            if msg_size == 0:
                ...
                if try_route_bracket_message(client_id, control_line):
                    continue
...
def handle_cctv_auto(conn, addr, client_id, prefetched_data=b""):
    """CCTV 계열: 초기 바이트로 영상/텍스트 자동 판별."""
    buf = bytearray(prefetched_data)
    conn.settimeout(2.0)

    # 초기 프로브
    PROBE_MAX = 16
    try:
        while len(buf) < 4 and len(buf) < PROBE_MAX:
    ...
    def looks_like_video_header(b: bytes) -> bool:
        if len(b) < 4:
            return False
        try:
            L = struct.unpack(">L", b[:4])[0]
        except Exception:
            return False
        return 0 <= L <= 10 * 1024 * 1024  # 0은 '컨트롤 라인' 프레임 용도로도 씀

    if looks_like_video_header(buf):
        print(f"CCTV 자동판별=영상 (ID:{client_id}, {addr})")
        # 영상/컨트롤(길이=0) 프로토콜은 기존 핸들러로 위임
        handle_client(conn, addr, client_id, bytes(buf))
        return

    print(f"CCTV 자동판별=텍스트 (ID:{client_id}, {addr})")
    ...
    try:
        ...
        while True:
            ...
            if not line:
                continue
            # ★ 대괄호 라우팅 우선
            if try_route_bracket_message(client_id, line):
                continue
            # 라우팅 규격이 아니면 그냥 로그
            print(f"[{client_id}] {line}")
    except OSError as exc:
        print(f"CCTV 텍스트 채널 오류({client_id}): {exc}")
    finally:
        unregister_text_client(client_id)
        print(f"CCTV 텍스트 채널 종료 (ID:{client_id})")
        conn.close()
...
def handle_user_client(conn, addr, client_id, prefetched_data=b""):
    ...
    try:
        while True:
            ...
            if try_route_bracket_message(client_id, message):
                continue
...
def start_socket_server():
    ...
    while True:
        ...
        if client_id.upper().startswith("USER"):
            ...
            thread = threading.Thread(target=handle_cctv_auto, args=(conn, addr, client_id, leftover))
        else:
