...
class Nav2Bridge(Node):
    ...
    def __init__(self, action_name: str = "navigate_to_pose", *, executor: Optional[MultiThreadedExecutor] = None) -> None:
        ...
        self._spin_client: Optional[ActionClient] = ActionClient(self, Spin, "spin")
    ...
    def spin(self, delta_yaw: float, timeout: float = 20.0) -> Nav2Result:
        """Spin 액션으로 제자리 회전을 수행."""
        if self._spin_client is None:
            raise RuntimeError("NAV2_SPIN_UNAVAILABLE")
        ...
        duration.nanosec = int((timeout - duration.sec) * 1_000_000_000)
        ...
        finished = complete_event.wait(timeout + 2.0)
        if not finished:
            self.get_logger().warning("Spin 액션이 타임아웃 되었습니다.")
            return Nav2Result(False, GoalStatus.STATUS_ABORTED, "SPIN_TIMEOUT")
